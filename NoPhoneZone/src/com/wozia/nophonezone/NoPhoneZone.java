...
public class NoPhoneZone extends TabActivity {
	...
	private static String msgAdd_Address, msgAdd_Footer;
	static final int QUIT_APP = 5;
	static final int NOTIFY_APP = 15;
	static final String APP_VERSION = "1.3.0";
    ...
    public void onCreate(Bundle savedInstanceState) {
     ...
     // Construct the LicenseCheckerCallback. The library calls this when done.
        ...
        msgAdd_Address = res.getString(R.string.nophonezone_msg_address);
	...
	public static void sendMessage(Context context, String PhoneNumber) {
		...
		//-- Check if number is in private group
		String[] tmp = { "", ""};
		tmp = dh.getContact(PhoneNumber);
		//Log.v("nophonezone", "Private PhoneNumber = " + PhoneNumber + ", tmp0 = " + tmp[0] + ", tmp1 = " + tmp[1]);
		if (tmp[0].length() > 0 || tmp[1].length() > 0) {
			addAddress = true;
		}
		if (sendMsg && addAddress) {
			gpsCoordinates = getGPS(context);
			//Geocoder g = new Geocoder(context, Locale.getDefault());
			//List<Address> addresses;
			String address = "";
			
			//Log.v("nophonezone", "gps coordinates before: " + gpsCoordinates[0] + ", " + gpsCoordinates[1]);
			
			//gpsCoordinates[0] = 41.2047;
			//gpsCoordinates[1] = -8.3284;
			
			//Log.v("nophonezone", "gps coordinates after: " + gpsCoordinates[0] + ", " + gpsCoordinates[1]);
			//Toast.makeText(context, "gps coordinate are " + gpsCoordinates[0] + ", " + gpsCoordinates[1], Toast.LENGTH_LONG).show();
			if (gpsCoordinates[0] != 0.0 || gpsCoordinates[1] != 0.0) {
				//try {
					//Log.v("nophonezone", "before getlocation");
					//addresses = g.getFromLocation(gpsCoordinates[0], gpsCoordinates[1], 1);
					Geocoder g = new Geocoder();
					l.setLatitude(gpsCoordinates[0]);
					l.setLongitude(gpsCoordinates[1]);
					address = g.reverseGeocode(l);
					//Toast.makeText(context, "address is " + address, Toast.LENGTH_LONG).show();
		            
					//Log.v("nophonezone", "after getlocation");
					//Log.v("nophonezone", "addresses = " + addresses.toString());
					//Log.v("nophonezone", "addresses = " + address);
					//if (!addresses.isEmpty()) {
					if (address.length() > 0) {
						currentAddress = address;//addresses.get(0).toString();
						//Log.v("nophonezone", "current address is: " + currentAddress);
		
						msgText += "\n" + String.format(NoPhoneZone.msgAdd_Address, currentAddress);
					}
				/*} catch (IOException e) {
					e.printStackTrace();
				}*/
