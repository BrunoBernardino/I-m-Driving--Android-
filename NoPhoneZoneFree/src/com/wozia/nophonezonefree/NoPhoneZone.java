...
public class NoPhoneZone extends TabActivity {
	...
	static final String APP_VERSION = "1.3.0 Free";
	...
	public static void sendMessage(Context context, String PhoneNumber) {
		...
		//msgText = formatCharset(msgText);
		//msgText = Html.fromHtml(msgText).toString();
		
		//Log.v("nophonezone", "debug: PhoneNumber is " + PhoneNumber);
		
		/*if (contactExists(context, PhoneNumber)) {
			Log.v("nophonezone", "debug: contact EXISTS");
		} else {
			Log.v("nophonezone", "debug: contact DOESN'T EXIST");
		}*/

		/*Log.v("nophonezone", "debug: msgText is " + msgText);
		Log.v("nophonezone", "debug: msgText.length() is " + msgText.length());
		if (sendMsg) {
			Log.v("nophonezone", "debug: sendMsg is TRUE");
		} else {
			Log.v("nophonezone", "debug: sendMsg is FALSE");
		}
		sendMsg = true;*/
		if (sendMsg) {
			//if (msgText.length() > 160) {
				...
				//Toast.makeText(context, "debug: option 1", Toast.LENGTH_LONG).show();
			/*} else {
				try {
				sm.sendTextMessage(PhoneNumber, null, msgText, null, null);
				} catch (IllegalArgumentException e) {
					e.printStackTrace();
					//Log.v("nophonezone","error exception = " + e.toString());
				}
				//Toast.makeText(context, "debug: option 2", Toast.LENGTH_LONG).show();
			}*/
			//Log.v("nophonezone", "message sent to " + PhoneNumber + "\n\n" + msgText);
